...
class EventProcessor:
    ...
    def mass(self, event):
	if event.totalWeight > MIN_WEIGHT:
		self.weights.append(event.totalWeight)
		if len(self.weights) > WEIGHT_SAMPLES:
			del self.weights[0]
			range = max(self.weights) - min(self.weights)
			if range < WEIGHT_THRESHOLD:
				sum = 0
				for weight in self.weights:
					sum += weight
				average = sum/len(self.weights)
				save_weight(average)
				print(average)
				self.weights = []
    @property
...
def save_weight(weight):
    ...
    file.write('{"weight":'+str(weight)+',"sent":false}')
    file.close()
